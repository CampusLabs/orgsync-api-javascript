...
assert.strictEqual(
  ...
  '/my/sweet/path/bro?test=sweet&id=bro&key=test'
);
...
assert.strictEqual(
  ...
  'https://api.orgsync.com/api/v3/my/sweet/path/bro?test=sweet&id=bro&key=test'
);

assert.strictEqual(
  api.url('/my/:test/path?special=:id', {test: 'sweet', id: 'bro'}),
  'https://api.orgsync.com/api/v3/my/sweet/path?special=bro&test=sweet' +
    '&id=bro&key=test'
);
